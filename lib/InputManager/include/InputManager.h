...
class InputManager {
 public:
  ...
  // True on the press edge of a GT911 capacitive home key (controllers without
  // one never report it). EEGO's GSL front key uses the short/long API below.
  bool wasHomeKeyPressed() const;
  // EEGO A4's dedicated front-panel capacitive key is intentionally split into
  // two gestures: a short click becomes Back and a hold becomes Home. These are
  // one-shot release/threshold events, respectively.
 ...
 private:
  ...
  // BoardConfig::ACTIVE.touch.controller (CHSC6x, GT911, or GSLX680).
  void beginTouch();
  ...
  bool touchHomeKeyEvent = false;  // GT911 key press edge
  bool touchHomeKeyShortEvent = false;
  bool touchHomeKeyLongEvent = false;
  bool touchHomeKeyDown = false;
  bool touchHomeKeyLongDelivered = false;
  unsigned long touchHomeKeyPressStart = 0;
  ...
  static constexpr unsigned long TOUCH_HOME_LONG_PRESS_MS = 700;
