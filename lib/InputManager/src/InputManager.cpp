...
uint8_t InputManager::getDigitalState() const {
  ...
  const int powerActiveLevel = BoardConfig::ACTIVE.input.powerActiveHigh ? HIGH : LOW;
  if (BoardConfig::ACTIVE.input.power >= 0 &&
      digitalRead(BoardConfig::ACTIVE.input.power) == powerActiveLevel &&
...
#if FREEINK_CAP_TOUCH
  return touchPressedEvent || touchReleasedEvent || touchHomeKeyEvent || touchHomeKeyDown ||
         touchHomeKeyShortEvent || touchHomeKeyLongEvent;
#else
  return false;
#endif
}
...
bool InputManager::wasHomeKeyShortPressed() const { return touchHomeKeyShortEvent; }
...
bool InputManager::gslx680Write(const uint8_t reg, const uint8_t* data, const uint8_t len) {
  ...
  Wire.beginTransmission(addr);
  Wire.write(reg);
  if (len != 0 && data != nullptr) Wire.write(data, len);
...
bool InputManager::gslx680Read(const uint8_t reg, uint8_t* data, const uint8_t len) {
  ...
  const uint8_t received = Wire.requestFrom(addr, len, static_cast<uint8_t>(true));
  if (received != len) {
    while (Wire.available()) Wire.read();
    return false;
  }
...
void InputManager::gslx680ClearRegisters() {
  ...
  delay(5);
  gslx680Write32(0xe0, 0x00000000);
  delay(20);
}
...
void InputManager::gslx680ResetChip() {
  ...
  gslx680Write32(0xe4, 0x00000004);
  delay(10);
  gslx680Write32(0xbc, 0x00000000);
  delay(10);
}
...
bool InputManager::gslx680LoadFirmware() {
  ...
  while (i < freeink::EEGO_A4_GSL_FIRMWARE_COUNT) {
    ...
    // The OEM uploader combines up to 16 consecutive 32-bit registers in one
    // transaction (64 data bytes plus the starting register).
    uint8_t payload[64];
    uint8_t words = 0;
    const uint8_t firstReg = entry.offset;
    while (i < freeink::EEGO_A4_GSL_FIRMWARE_COUNT && words < 16) {
      memcpy_P(&entry, &freeink::EEGO_A4_GSL_FIRMWARE[i], sizeof(entry));
      const uint8_t expected = static_cast<uint8_t>(firstReg + words * 4);
      if (entry.offset == 0xf0 || entry.offset != expected) break;
      payload[words * 4] = static_cast<uint8_t>(entry.value);
      payload[words * 4 + 1] = static_cast<uint8_t>(entry.value >> 8);
      payload[words * 4 + 2] = static_cast<uint8_t>(entry.value >> 16);
      payload[words * 4 + 3] = static_cast<uint8_t>(entry.value >> 24);
      ++words;
      ++i;
    }
    if (words == 0 || !gslx680Write(firstReg, payload, static_cast<uint8_t>(words * 4))) return false;
  }
  return true;
#else
  return false;
...
bool InputManager::gslx680Check() {
  uint8_t status[4] = {};
  if (!gslx680Read(0xb0, status, sizeof(status))) return false;
...
void InputManager::beginGslx680() {
  ...
  // Probe and exercise the page-select register exactly as CrossLink does.
  uint8_t probe = 0;
  if (!gslx680Read(0xf0, &probe, 1) || !gslx680Write32(0xf0, 0x00000012) ||
      !gslx680Read(0xf0, &probe, 1)) {
  ...
  touchDebugPrintf("[touch] GSLX680 probe: addr=0x%02X enabled=%d (sda=%d scl=%d)\n", t.i2cAddress,
                   touchDataEnabled, t.sda, t.scl);
#endif
}
...
void InputManager::pollGslx680(const unsigned long now) {
  ...
  const auto finishHomeKey = [this, now]() {
    ...
    const unsigned long heldMs = now - touchHomeKeyPressStart;
    lastTouchHeldDurationMs = heldMs;
    if (!touchHomeKeyLongDelivered) touchHomeKeyShortEvent = true;
    touchHomeKeyDown = false;
    touchHomeKeyLongDelivered = false;
  };
  ...
  // GSL point 0: Y in bytes 4/5, X in bytes 6/7. Official 1.2.7 reserves the
  // otherwise out-of-range single-point pair X=0x03a0/Y=0x1020 for its
  // front-panel "NotificationShade" touch key. Detect it before stripping the
  // high nibble; otherwise it aliases to an ordinary near-corner coordinate.
  ...
  if (homeKeyDown) {
    if (!touchHomeKeyDown) {
      ...
      touchHomeKeyLongDelivered = false;
      touchHomeKeyPressStart = now;
    } else if (!touchHomeKeyLongDelivered && now - touchHomeKeyPressStart >= TOUCH_HOME_LONG_PRESS_MS) {
      touchHomeKeyLongEvent = true;
      touchHomeKeyLongDelivered = true;
    }
    if (touchPressed) {
      touchReleasedEvent = true;
      lastTouchHeldDurationMs = now - touchDownPoint.timestamp;
  ...
  // Exact integer calibration recovered from the official 1.2.7 firmware's
  // FUN_4204c510. It supersedes CrossLink 1.0.10's narrower/offset transform,
  // which only covered ~536 of the 552 short-axis pixels:
  //   portraitX = min(rawY, 680) * 551 / 680
  //   portraitY = (920 - min(rawX, 920)) * 767 / 920
  // The digitizer is portrait while the UC8279C framebuffer is landscape, so
  // swap the calibrated axes when returning panel-native coordinates.
  ...
  touchPoint = {true, portraitY, static_cast<uint16_t>(551 - portraitX), now};
  if (!touchPressed) {
    touchPressedEvent = true;
