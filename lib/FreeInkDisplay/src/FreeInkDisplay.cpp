...
#endif
{
  // The public constructor keeps the EInkDisplay signature. BoardConfig is the
  // authoritative pin source for every runtime-selected panel.
  (void)sclk;
  (void)mosi;
  (void)cs;
  (void)dc;
...
#elif FREEINK_DRIVER_UC8279C_A4
...
void FreeInkDisplay::begin() {
  ...
  if (!_driver->usesExternalBus()) {
    ...
    // so one binary drives the runtime-selected panel with the corresponding
    // board wiring, including EPD power-enable.
