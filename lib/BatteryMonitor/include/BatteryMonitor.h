...
//     a percentage; an optional charge-status pin drives isCharging(), with
//     polarity supplied by BoardConfig (usually active-low).
...
public:
    ...
    //   * ADC boards: the profile's charge-status pin and polarity; always false
    //     when no charge-status pin is configured.
