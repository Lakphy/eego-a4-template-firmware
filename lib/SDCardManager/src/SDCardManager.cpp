...
bool SDCardManager::begin() {
  ...
  if (BoardConfig::ACTIVE.sd.separateSpi &&
      SD_SCLK >= 0 && SD_MOSI >= 0 && SD_MISO >= 0) {
    // Keep a genuinely separate controller alive for the lifetime of SdFat.
    // Calling begin() twice on Arduino's global SPI object does not remap it:
    // SPIClass::begin() returns immediately once its bus is started. On EEGO A4
    // that left the display's SCLK/MOSI commands on the SD pins, so the UI drew
    // normally in RAM while the physical EPD retained its pre-boot image.
    static SPIClass dedicatedSdSpi(HSPI);
    cardReady = dedicatedSdSpi.begin(SD_SCLK, SD_MISO, SD_MOSI, SD_CS) &&
                sd.begin(SdSpiConfig(SD_CS,
                                     DEDICATED_SPI | USER_SPI_BEGIN,
                                     SPI_FQ,
                                     &dedicatedSdSpi));
  } else {
