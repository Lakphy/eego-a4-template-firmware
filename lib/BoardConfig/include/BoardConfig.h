...
// --- 2) Coherence: exactly one MCU family, at least one device ---------------
      ...
      FREEINK_DEVICE_LILYGO || FREEINK_DEVICE_M5PAPER || FREEINK_DEVICE_STICKY || FREEINK_DEVICE_EEGO_A4)
#error \
    "FreeInk: no device selected. Pass at least one -DFREEINK_DEVICE_<NAME> (X4, X3, M5, MURPHY, DELINK, LILYGO, M5PAPER, STICKY, EEGO_A4) in your build env — see platformio.sample.ini."
#endif
...
#define FREEINK_MCU_S3 \
  (FREEINK_DEVICE_M5 || FREEINK_DEVICE_MURPHY || FREEINK_DEVICE_DELINK || FREEINK_DEVICE_LILYGO || \
   FREEINK_DEVICE_STICKY || FREEINK_DEVICE_EEGO_A4)
...
#define FREEINK_CAP_FRONTLIGHT \
  (FREEINK_DEVICE_DELINK || FREEINK_DEVICE_MURPHY || FREEINK_DEVICE_LILYGO || FREEINK_DEVICE_EEGO_A4)
#endif
...
// addresses. Set -DFREEINK_BLE_HID_SHOW_UNNAMED_DEVICES=1 during diagnostics to
...
#define FREEINK_CAP_RTC (FREEINK_DEVICE_X3 || FREEINK_DEVICE_STICKY || FREEINK_DEVICE_EEGO_A4)
#endif
...
// while Sticky logs are more reliable through the IDF/ROM console path.
...
enum class DisplayController : uint8_t { SSD1677, UC8253, UC8279C, ED2208, LgfxEpd, IT8951 };

// Optional capacitive touch controller.
enum class TouchController : uint8_t { None, Chsc6x, Gt911, Gslx680 };
...
// contract comes from the official pin map and M5Unified's speaker configuration.
...
// I2C LED driver frontlight. EEGO A4 uses an LM3630A instead of a PWM GPIO.
...
struct BoardProfile {
  ...
  PowerConfig power = {};
  // Optional I2C frontlight controller, separate from the GPIO PWM path.
  I2cFrontlightConfig i2cFrontlight = {
      I2cFrontlightController::None, PIN_UNASSIGNED, PIN_UNASSIGNED, 0, 0, PIN_UNASSIGNED};
  // Most charger /STAT pins are active-low. Polarity is board data instead of
  // driver guesswork; optional hardware variants may still need a runtime
  // override in their board-specific facade.
  bool batteryChargeActiveHigh = false;
};
...
// audio-specific, but the OEM initialization keeps it high). GPIO46 carries
...
// PaperColor) and M5Unified's speaker configuration: ES8311 mono codec at 7-bit I2C
...
constexpr BoardProfile STICKY = {
    ...
    // track-id), like M5Paper — confirmed by raw point dumps.
    // Portrait digitizer on a landscape panel: swapXY + flip both maps the sensor
    // frame onto the panel-native frame (confirmed by corner and menu touch samples).
...
// --- EEGO A4 — ESP32-S3, UC8279C, GSLX680 touch ------------------------------
// Recovered from the public CrossLink full/app firmware (v1.0.8/v1.0.10). The
// controller scans 768x600: the OEM framebuffer contains 552 rows and appends
// 48 white padding rows on every RAM write. Product literature describes a
// 768x528 visible area; the OEM touch/UI calibration uses 768x536. We preserve
// the binary's actual 768x552 framebuffer contract and let the panel mask hide
// the non-visible gate rows.
constexpr BoardProfile EEGO_A4 = {
    ...
    768,
    552,
    // UC8279C SPI: SCLK42 MOSI45 CS21 DC14 RST13 BUSY41; GPIO6 gates the
    // display rail and is driven HIGH by the OEM at boot, LOW+held in sleep.
    {42, 45, 21, 14, 13, 41, 6},
    20000000,
    // Dedicated SD SPI bus recovered from SDCardManager::begin().
    {39, 40, 38, 47, PIN_UNASSIGNED, true, 20000000},
    // Side keys are active-low; the power key is active-high.
    {PIN_UNASSIGNED, PIN_UNASSIGNED, PIN_UNASSIGNED, PIN_UNASSIGNED, 5, 7, 8, true},
    10,    // battery ADC
    11,    // charge-status input; no-frontlight retail unit is active-low
    1.559f,
    PIN_UNASSIGNED,
    // GSLX680 at 0x40 on the shared 400-kHz I2C bus. Its dedicated backend
    // applies the official 1.2.7 calibration and returns the complete
    // panel-native x=0..767, y=0..551 range. This corrects CrossLink 1.0.10's
    // narrower ~536-row calibration, which made edge/front-key hits inaccurate.
    {TouchController::Gslx680, 2, 1, PIN_UNASSIGNED, PIN_UNASSIGNED, 0x40,
     0, 767, 0, 551, false, 0, false, false},
    NO_FRONTLIGHT,  // LM3630A uses the trailing I2C-frontlight config
    NO_AUDIO,
    NO_LEDS,
    NO_FLIP,
    NO_SDMMC,
    NO_GAUGE,
    NO_MIC,
    // PCF8563 shares SDA2/SCL1 with GSLX680 and LM3630A.
    {2, 1, 400000, 0x51, 0, 0, 0, RtcType::Pcf8563, ImuType::None},
    1.2f,
    // GPIO4 is driven/held HIGH by the OEM in both run and sleep states.
    {4},
...
constexpr uint32_t MAX_FRAMEBUFFER_BYTES =
    cmax(cmax(cmax(cmax(FREEINK_DEVICE_X4 ? panelBytes(XTEINK_X4) : 0u,
